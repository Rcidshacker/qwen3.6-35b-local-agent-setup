...
- **Fastest daily driver — REAP-28B:** the pruned `Qwen3.6-28B-REAP20-A3B` runs **+40–70% faster decode** at matched/better quality (verified A/B) — **not** because it streams less per token (a GGUF tensor dump shows it actually streams ~7% *more*), but because its smaller expert *pool* (205 vs 256) keeps a larger fraction GPU-resident under `--fit`, cutting real PCIe fetches. `start_agent.ps1` defaults to it (with `-ub 4096`), falling back to the 35B. See [§15](LOCAL_LLM_SETUP_REPORT.md) / [§16](LOCAL_LLM_SETUP_REPORT.md).
